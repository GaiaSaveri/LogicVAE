...
class GAT(nn.Module):
    def __init__(self, num_layers, num_heads_layer_list, num_features_layer_list, device):
        super(GAT, self).__init__()
        num_heads_layer = [1] + num_heads_layer_list
        self.gat_layers = nn.ModuleList()
        for i in range(num_layers):
            layer = GATLayer(in_dim=num_features_layer_list[i]*num_heads_layer[i],
                             out_dim=num_features_layer_list[i+1],
